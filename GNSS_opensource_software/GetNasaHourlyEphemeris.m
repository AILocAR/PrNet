function [allGpsEph,iono] = GetNasaHourlyEphemeris(utcTime,dirName)
...
%       iono is the ionospheric delay correction parameters
%
...
yearNumber2Digit = rem(utcTime(1),100);%the last two digits of the year
dayNumber = DayOfYear(utcTime);

hourlyZFile = sprintf('hour%03d0.%02dn.Z',dayNumber,yearNumber2Digit);
ephFilename = hourlyZFile(1:end-2);%discard the suffix .Z
% For data after December 1, 2020, the files are gzipped with extension .gz
if yearNumber4Digit>2020 || (yearNumber4Digit ==2020 && dayNumber > 335) 
    hourlyZFile = sprintf('hour%03d0.%02dn.gz',dayNumber,yearNumber2Digit);
    ephFilename = hourlyZFile(1:end-3);%discard the suffix .gz
end
...
%     [allGpsEph,iono] = ReadRinex3NavGps(fullEphFilename);
...
if ~bGotGpsEph
    ...
    if ~strfind(dirName,'com~apple~CloudDocs') % exclude the user who uses iCloud on Mac
        if strfind(dirName,'~')
            fprintf('\nYou set: dirName = ''%s''\n',dirName)
...
% allGpsEph = ReadRinex3NavGps(fullEphFilename);
