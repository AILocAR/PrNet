function [gnssRaw,gnssAnalysis] = ReadGnssLoggerNew(dirName,fileName,dataFilter,gnssAnalysis)
...
[gnssRaw.GPS_la,gnssRaw.GPS_lg,gnssRaw.GPS_am] = ReadXLPCsv(dirName + "/GPS.csv");
[gnssRaw.NLP_la,gnssRaw.NLP_lg,gnssRaw.NLP_am] = ReadXLPCsv(dirName + "/NLP.csv");
[gnssRaw.FLP_la,gnssRaw.FLP_lg,gnssRaw.FLP_am] = ReadXLPCsv(dirName + "/FLP.csv");
...
% Firstly,find the version number
line='';
...
if line==-1 %Read the end line of the file
    fprintf('\nCould not find "Version" in input file %s\n',fileName)
    return
end

%look for the beginning of the version number, e.g. 1.4.0.0
iDigits = regexp(line,'\d'); %index into the first number found in line, '\d' regular expression:0~9

%Extract version number and platform
%eg.
...
csvGPSFileName = [dirName,'GPS.csv'];
csvGPSfileID = fopen(csvGPSFileName,'w');
csvNLPFileName = [dirName,'NLP.csv'];
csvNLPfileID = fopen(csvNLPFileName,'w');
csvFLPFileName = [dirName,'FLP.csv'];
csvFLPfileID = fopen(csvFLPFileName,'w');
while ischar(line)
    line = fgetl(txtfileID);
    if ~isempty(strfind(line,'Raw,'))      
        ...
        line = strrep(line,'Raw,',''); %remove 'Raw'; for data and header
        line = strrep(line,'#',''); line = strrep(line,' ','');%remove '#' and spaces; for header
    ...
    elseif ~isempty(strfind(line,'GPS,'))
        line = strrep(line,'GPS,',''); %remove 'GPS'; for data
        line = strrep(line,'Fix,',''); %remove 'Fix'; for data
        %only write LLA
        newline = split(line,",");
        comline = [newline{1},',',newline{2},',',newline{3}];
        fprintf(csvGPSfileID,'%s\n',comline);
    elseif ~isempty(strfind(line,'NLP,'))
        line = strrep(line,'NLP,',''); %remove 'NLP'; for data
        line = strrep(line,'Fix,',''); %remove 'Fix'; for data
        newline = split(line,",");
        comline = [newline{1},',',newline{2},',',newline{3}];
        fprintf(csvNLPfileID,'%s\n',comline);
    elseif ~isempty(strfind(line,'FLP,'))
        line = strrep(line,'FLP,',''); %remove 'NLP'; for data
        line = strrep(line,'Fix,',''); %remove 'Fix'; for data
        newline = split(line,",");
        comline = [newline{1},',',newline{2},',',newline{3}];
        fprintf(csvFLPfileID,'%s\n',comline); 
    else
        continue;
    end
end
...
for i=1:numFields
    ...
    elseif i == find(strcmp(header,'TimeNanos')) || ...
            ...
            i == find(strcmp(header,'BiasUncertaintyNanos')) || ...
            i == find(strcmp(header,'ReceivedSvTimeNanos')) || ...
            i == find(strcmp(header,'ReceivedSvTimeUncertaintyNanos')) || ...
            i == find(strcmp(header,'DriftUncertaintyNanosPerSecond')) || ...
    ...
    elseif 0
        ...
        formatSpec = sprintf('%s %%f',formatSpec);%åœ¨formatSpecå?Žé?¢ä¸?æ–­è¿½åŠ 
    end
end
%Replace empty string fields with 'NaN'
C = textscan(fid,formatSpec,'Delimiter',',','EmptyValue',NaN);

% Add index to the end column
C{1, end+1} = (1:length(C{1,1}))';
...
function [la,lg,Am] = ReadXLPCsv(XLPCsvFile)
fid = fopen(XLPCsvFile);
if fid<0
    error('file ''%s'' not found',XLPCsvFile);
end
...
assert(length(C)==length(header)+1,...
    'length(C) ~= length(header)+1. This should have been checked before here')
...
for j = 1:length(header)
    if header{j} == "CodeType"
        eval(['gnssRaw.',header{j}, '=C{j};']);
    elseif any(isfinite(C{j})) %not all NaNs
...
assert(~bChangeSign,...
    'FullBiasNanos changes sign within log file, this should never happen');% If assert is failed, the program will end here.
...
%compute full cycle time of measurement, in milliseonds w.r.t. GPS time
