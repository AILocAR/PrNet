function [xHat,z,svPos,H,Wpr,Wrr,R_Ac,I_iono_logger,I_trop_logger] = WlsPvt(prs,gnssEph,xo,iono, n)
...
%   xo: initial (previous) state, [x,y,z,bc,xDot,yDot,zDot,bcDot]'
...
[bOk,numVal] = checkInputs(prs, gnssEph, xo);
if ~bOk
...
% [svXyzTtx,dtsv,svXyzDot,dtsvDot]=BdsEph2Pvt(gnssEph,[ttxWeek,ttx]);
svXyzTrx = svXyzTtx; %initialize svXyz at time of reception

%Compute weights ---------------------------------------------------
% Wpr = eye(length(prs(:,jPrSig)));
% Wrr = eye(length(prs(:,jPrrSig)));
...
while norm(dx) > GnssThresholds.MAXDELPOSFORNAVM
  ...
  %% Calculate Pesudorange Residuals
  ...
  zPr = prs(:,jPr)-prHat; % The pseudorange measurements - pseudorange_hat
...
prrHat = rrMps - GpsConstants.LIGHTSPEED*dtsvDot;
...
RrMps = zeros(numVal,1);
for i=1:numVal
    %range rate = [user velocity - satellite velocity] dot product [los from xo to sv]
    RrMps(i) = (vHat(1:3)'-svXyzDot(i,:))*v(:,i);
end

svPos=[prs(:,3),svXyzTrx,dtsv(:),dtsvDot(:),RrMps];
